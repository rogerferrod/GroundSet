...
class EvalDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for match in search:
            ...
            x_left, y_top, x_right, y_bottom = nums
            x = (x_left + x_right) / 2
            y = (y_top + y_bottom) / 2
            w = x_right - x_left
            h = y_bottom - y_top

            rbox = '<rbox>({' + f'<{x}><{y}><{w}><{h}>|<90>' + '})</rbox>'
...
def run(args, config):
    ...
    disable_torch_init()
    model_name = 'geochat-' + get_model_name_from_path(args.model)
...
def main():
    ...
    parser.add_argument('-m', '--model', type=str, default='ll-13/SkySenseGPT-7B-CLIP-ViT')
    parser.add_argument('-b', '--batch', type=int, default=1)
    parser.add_argument('-o', '--output', type=str, default='../output/eval/vrs_skysense')
