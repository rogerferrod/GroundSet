...
def run(args):
    ...
    dfs = []
    files = [x for x in glob.glob(os.path.join(args.input, '*.csv')) if re.search(r'detection_[\d]+\.csv$', x)]
    for file in files:
    ...
    with open(os.path.join(args.output, 'detection_scores.txt'), 'w') as out:
...
def main():
    ...
    if model not in {'llava', 'llava-next', 'gemma', 'geochat', 'minigpt', 'skysense', 'ferret', 'paligemma', 'gemini',
                     'vrsbench', 'finetune'}:
